...
client = AerClient(backend=LocalProcessBackend(max_workers=4))
artifacts = client.execute_tasks(tasks)

# Parallel threads — best for I/O-bound extractors (e.g. COG readers)
client = AerClient(backend=ThreadBackend(max_workers=8))
artifacts = client.execute_tasks(tasks)
```
...
from aer.execution.core import TaskStaging
